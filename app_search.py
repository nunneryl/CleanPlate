# app_search.py - LIVE Production Version with Full Synonym Map
...
# Local application imports (assuming they exist in your main branch)
try:
...
# This map is generated from your provided PDF and spreadsheet.
...
# --- Normalization Function (From your live v1.2) ---
...
# --- Search Endpoint (From your live v1.2) ---
...
def search():
    ...
    # This is the query structure from your live app.
    # It does not include the new grade/boro/sort parameters.
    query = """
    WITH latest_restaurants AS (
        ...
        ORDER BY CASE WHEN dba_normalized_search = %s THEN 0 WHEN dba_normalized_search ILIKE %s THEN 1 ELSE 2 END,
    ...
    ORDER BY CASE WHEN pc.dba_normalized_search = %s THEN 0 WHEN pc.dba_normalized_search ILIKE %s THEN 1 ELSE 2 END,
    similarity(pc.dba_normalized_search, %s) DESC, length(pc.dba_normalized_search), pc.dba ASC, r.inspection_date DESC;
    """
